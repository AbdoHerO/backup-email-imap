    ...
    for index, email_id in enumerate(email_ids, start=1):
        ...
        for response_part in msg_data:
            if isinstance(response_part, tuple):
                ...
                save_email_and_attachments(msg, index, backup_dir)
                save_to_mbox(mbox, msg)

    mbox.close()


def save_email_and_attachments(msg, index, backup_dir):
    """Save an email to a file and extract its attachments."""
    ...
    email_filename = f"email_{index}_{sanitized_subject[:50]}.eml"  # Limit filename length
    email_filepath = os.path.join(backup_dir, email_filename)

    # Save the email content to a file
    with open(email_filepath, "wb") as f:
        f.write(msg.as_bytes())
    print(f"Saved email: {email_filepath}")

    # Create a folder for attachments of this email
    attachments_dir = os.path.join(backup_dir, f"email_{index}_attachments")
    ...
    for part in msg.walk():
        ...
        if filename:
            ...
            attachment_filepath = os.path.join(attachments_dir, filename)
            with open(attachment_filepath, "wb") as f:
                f.write(part.get_payload(decode=True))
            print(f"Saved attachment: {attachment_filepath}")
